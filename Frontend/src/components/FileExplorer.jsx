...
function FileIcon({ name }) {
  ...
  return (
    <span className={`text-xs font-mono font-bold w-4 text-center ${color}`}>
      {icons[ext] || '·'}
    </span>
  );
}
  ...
  if (isFile) {
    return (
      <button
        ...
        title={node.__path}
        className={`w-full flex items-center gap-1.5 px-2 py-1 rounded text-left text-xs transition-all duration-150
          ${isSelected
  ...
  return (
    <div>
      <button
        ...
        className="w-full flex items-center gap-1.5 px-2 py-1 rounded text-left text-xs text-slate-500 hover:text-slate-300 hover:bg-white/4 transition-all duration-150"
        style={{ paddingLeft: `${8 + depth * 12}px` }}
      >
        <svg
        ...
        <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" className="text-slate-600 flex-shrink-0">
...
export default function FileExplorer({ sandboxId }) {
  ...
  return (
    <div className="flex h-full bg-[#0a0a0f] overflow-hidden">
      {/* Sidebar */}
      <div className="w-56 flex-shrink-0 flex flex-col border-r border-white/5 bg-[#0f0f1a]">
        {/* Header */}
        <div className="flex items-center justify-between px-3 py-2 border-b border-white/5">
          ...
          <button
            onClick={fetchFiles}
            title="Refresh files"
            className="p-1 rounded text-slate-600 hover:text-slate-400 hover:bg-white/5 transition-all duration-150"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {selectedFile ? (
          <>
            {/* File tab */}
            <div className="flex items-center gap-2 px-4 py-2 border-b border-white/5 bg-[#0f0f1a]">
            ...
            <div className="flex-1 overflow-auto">
              {contentLoading ? (
                <div className="flex items-center justify-center h-full">
                  <span className="w-6 h-6 border-2 border-indigo-500/30 border-t-indigo-500 rounded-full animate-spin-slow" />
                </div>
              ) : (
